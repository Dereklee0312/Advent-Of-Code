...
def move2(grid, curRow, curCol, maxRow, maxCol):
    """
    Part 2
    Instead of searching shortest path from each `a` elevation to `E`,
    Start from `E` to first elevation of `a`
    """
    # Moves that can be made
    # I.e, only horizontal and vertical
    movements = [(0, -1), (0, 1), (-1, 0), (1, 0)]
    ...
    while queue:
        ...
        for i, j in movements:
            if 0 <= start.row + i < maxRow and 0 <= start.col + j < maxCol:
                next = grid[start.row + i][start.col + j]
                # Check if next position is viable and unvisited
                if start.value - next.value <= 1 and next.visited == False:
                    next.visited = True
                    queue.append((next, count + 1))
